from diffusers import StableDiffusionPipeline, StableDiffusionXLPipeline, IFPipeline
from typing import List, Dict, Callable, Union
import torch
from .hooked_scheduler import HookedNoiseScheduler


def _cache_tensor(io):
    """Convert cached hook tensors to detached CPU tensors to avoid GPU OOM."""
    tensor = retrieve(io)
...
class HookedDiffusionAbstractPipeline:
    ...
    def from_pretrained(cls, *args, **kwargs):
        use_hooked_scheduler = kwargs.pop("use_hooked_scheduler", False)
        return cls(cls.parent_cls.from_pretrained(*args, **kwargs), use_hooked_scheduler=use_hooked_scheduler)

    @classmethod
    def from_single_file(cls, *args, **kwargs):
        """
        Load a diffusion model from a single checkpoint file (e.g. `.ckpt` / `.safetensors`).

        This is a thin wrapper around diffusers' `Pipeline.from_single_file(...)` when available.
        """
        use_hooked_scheduler = kwargs.pop("use_hooked_scheduler", False)
        if not hasattr(cls.parent_cls, "from_single_file"):
            raise AttributeError(
                f"{cls.parent_cls.__name__} does not support from_single_file in this diffusers version."
            )
        return cls(cls.parent_cls.from_single_file(*args, **kwargs), use_hooked_scheduler=use_hooked_scheduler)
    ...
    def run_with_timed_hook(self, 
        *args,
        positions_to_cache: List[str],
        timesteps_to_fire: List[int],
        ...
        **kwargs
        ...
        Returns:
            ...
            cache_dict (Dict[str, Dict[str, Any]]): A dictionary where keys are the specified positions 
                ...
                depending on the flags `save_input` and `save_output`.
        '''
        timer = {"now": 0, "fire_at": timesteps_to_fire}
        hooks = [
            self._register_timed_hook(position, timer, func_to_call) for position in positions_to_cache
        ]
        hooks = [hook for hook in hooks if hook is not None]
        output = self.pipe(*args, **kwargs)
        for hook in hooks:
    ...
    def _register_cache_hook(
        ...
        cache_input: Dict,
        cache_output: Dict,
        timestep_cache_set: set[int] | None = None,
    ):
        ...
        if position == 'noise':
            def hook(model_output, timestep, sample, generator):
                ...
                cache_output[position].append(sample.detach().to(device="cpu"))
        ...
        def hook(module, input, kwargs, output):
            # step_idx：当前 diffusion 走到第几步（对每个 position 来说，每步会触发一次 hook）
            # 用 list 的长度作为 step 计数，避免多个 position 共享计数导致错位。
            counter_list = None
            if cache_output is not None:
                if position not in cache_output:
                    cache_output[position] = []
                counter_list = cache_output[position]
            elif cache_input is not None:
                if position not in cache_input:
                    cache_input[position] = []
                counter_list = cache_input[position]

            if counter_list is not None and timestep_cache_set is not None:
                step_idx = len(counter_list)
                if step_idx not in timestep_cache_set:
                    return

            if cache_input is not None:
                if position not in cache_input:
                    cache_input[position] = []
                cache_input[position].append(_cache_tensor(input))
            
            if cache_output is not None:
                if position not in cache_output:
                    cache_output[position] = []
                cache_output[position].append(_cache_tensor(output))

        return block.register_forward_hook(hook, with_kwargs=True)
    
    def _register_timed_hook(self, position: str, timer: Dict, func: Callable):
        block = self._locate_block(position)

        def hook(module, input, kwargs, output):
            if timer["now"] in timer["fire_at"]:
                is_output_tuple = isinstance(output, tuple)
                out = func(module, retrieve(input), kwargs, retrieve(output))
                if is_output_tuple:
                    out = (out,)
            else:
                out = output
