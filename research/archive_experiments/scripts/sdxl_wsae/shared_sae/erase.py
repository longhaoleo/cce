...
from sdxl_wsae.core.intervention import (
    InterventionSpec,
    _conditional_slice,
    ...
    _flatten_spatial,
    _in_time_window,
    _maybe_apply_spatial_mask,
    _maybe_apply_spatial_norm_weight,
    _pack_tensor,
    _resolve_feature_list,
    _unflatten_spatial,
)
from sdxl_wsae.shared_sae.intervention_utils import (
    _build_block_scale_map,
    _interpolate_coeff_by_step,
    _load_coeff_by_step_from_exp53_csv,
    _save_hook_debug_csv,
)
from sdxl_wsae.shared_sae.scoring import _load_blacklist_ids
from sdxl_wsae.utils import block_short_name, ensure_dir, extract_first_image, safe_name
...
class SpatialInterventionConfig:
    ...
    use_norm_weight: bool = False
    mask: str = "none"
    mask_sigma: float = 0.25
...
def build_parser() -> argparse.ArgumentParser:
    ...
    g_int.add_argument("--targetconcept", type=str, required=True, help="概念名，对应 concept_root/<block>/<concept>/。")
    g_int.add_argument(
        "--blocks",
        nargs="+",
        type=str,
        default=None,
        help="干预 block 列表；默认使用 checkpoint 里的 block。",
    )
    g_int.add_argument(
        "--int_mode",
        type=str,
        default="ablation",
        choices=["injection", "ablation"],
        help="概念操作模式。",
    )
    g_int.add_argument("--int_scale", type=float, default=100.0, help="全局干预强度。")
    g_int.add_argument("--int_feature_top_k", type=int, default=10, help="从 top_positive_features.csv 读取前 K 个特征。")
    g_int.add_argument(
        "--int_use_time_weight",
    ...
    g_int.add_argument("--int_t_start", type=int, default=1000, help="干预时间窗上界（高噪声侧）。")
    g_int.add_argument("--int_t_end", type=int, default=0, help="干预时间窗下界（低噪声侧）。")
    g_int.add_argument("--int_step_start", type=int, default=-1, help=">=0 时启用 step 下界。")
    g_int.add_argument("--int_step_end", type=int, default=-1, help=">=0 时启用 step 上界。")
    g_int.add_argument(
    ...
    g_int.add_argument("--int_mask_sigma", type=float, default=0.25, help="空间高斯 mask 的 sigma 相对尺度。")
    g_int.add_argument(
...
def _build_intervention_cfg(args: argparse.Namespace) -> SharedInterventionConfig:
    ...
    return SharedInterventionConfig(
        ...
        spatial=SpatialInterventionConfig(
            ...
            mask=str(args.int_spatial_mask),
...
def _build_intervention_spec(
    *,
    ...
    return InterventionSpec(
        ...
        spatial_mask=str(cfg.spatial.mask),
        mask_sigma=float(cfg.spatial.mask_sigma),
...
def main() -> None:
    ...
    coeffs_by_block = {
        block: features.coeff_by_step for block, features in features_by_block.items()
    }
