...
from sdxl_wsae.shared_sae.intervention_utils import _build_block_scale_map, _save_hook_debug_csv
from sdxl_wsae.utils import ensure_dir, extract_first_image, safe_name
...
from .erase import (
    ...
    SpatialInterventionConfig,
    TimeInterventionConfig,
...
def parse_args() -> argparse.Namespace:
    ...
    g_int.add_argument(
        ...
        help="默认使用当前更稳定的三层；如需消融可显式传入。",
    )
    g_int.add_argument("--int_mode", type=str, default="ablation", choices=["injection", "ablation"])
    g_int.add_argument("--int_scale", type=float, default=20.0)
    g_int.add_argument("--int_feature_top_k", type=int, default=5)
    g_int.add_argument("--int_use_time_weight", action=argparse.BooleanOptionalAction, default=False)
    ...
    g_int.add_argument("--int_spatial_mask", type=str, default="none", choices=["none", "gaussian_center"])
    ...
    if p.suffix.lower() == ".csv":
        with p.open("r", encoding="utf-8") as f:
            ...
            for idx, row in enumerate(reader):
                ...
                if not prompt:
                    continue
                case_number = int(row["case_number"]) if has_case and str(row.get("case_number", "")).strip() else idx
                if has_seed and str(row.get(seed_col, "")).strip():
...
def main() -> None:
    ...
    with open(manifest_path, "w", encoding="utf-8") as mf:
        ...
        for concept in concepts:
            ...
            coeffs_by_block = {block: feat.coeff_by_step for block, feat in features_by_block.items()}
